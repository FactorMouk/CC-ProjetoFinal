...
const sketch = (p5: P5) => {
  function lines() {
    for (let x = 0; x < points.length; x++) {
      ...
      const cwhite = (p5.red(c) + p5.green(c) + p5.blue(c)) / slider2.value();
      const ct = image.get(points[x].x, points[x].y - 1);
      const ctwhite =
        (p5.red(ct) + p5.green(ct) + p5.blue(ct)) / slider2.value();
      const cr = image.get(points[x].x + 1, points[x].y);
      const crwhite =
        (p5.red(cr) + p5.green(cr) + p5.blue(cr)) / slider2.value();
      const cb = image.get(points[x].x, points[x].y + 1);
      const cbwhite =
        (p5.red(cb) + p5.green(cb) + p5.blue(cb)) / slider2.value();
      const cl = image.get(points[x].x - 1, points[x].y);
      const clwhite =
        (p5.red(cl) + p5.green(cl) + p5.blue(cl)) / slider2.value();
      const edge = Math.max(
        cwhite - ctwhite,
        cwhite - crwhite,
        cwhite - cbwhite,
  ...
  p5.setup = () => {
    function createCanvas() {
      ...
      slider3.position(image.width + 250, 150);
      slider3.style('width', '130px');

      button = p5.createButton('Wave');
      button.position(image.width + 250, 200);
      button.mousePressed(() => {
        isWaving = !isWaving;
      });
    }
